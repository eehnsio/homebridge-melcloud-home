...
export class MELCloudAPI {
  ...
  // Last-known-good token state, tracked in memory so a refresh_failure entry
  // can be self-contained (see AuthAuditLog). Successes are never written to disk.
  private readonly configTokenSuffix: string;
  private lastRotatedSuffix?: string;
  private lastPersistedSuffix?: string;
  ...
  constructor(config: MELCloudConfig) {
    ...
    this.configTokenSuffix = maskToken(config.refreshToken);
  }
   ...
   */
  private failureContext(usedTokenSuffix: string) {
    return {
      fromConfigToken: usedTokenSuffix === this.configTokenSuffix,
   ...
   */
  private familyAgeDays(): number | undefined {
   ...
   */
  adoptTokens(tokens: { refreshToken: string; accessToken: string; expiresIn: number }, familyStartedAt: string): void {
    ...
    this.lastRotatedSuffix = maskToken(tokens.refreshToken);
    this.lastPersistedSuffix = maskToken(tokens.refreshToken);
      ...
      const req = https.request(options, (res) => {
        ...
        res.on('end', async () => {
          ...
          try {
            ...
            // Track last-known-good rotation in memory (not logged on success).
            if (rotated) {
              this.lastRotatedSuffix = newTokenSuffix;
            }
            ...
            if (this.config.onTokenRefresh && tokenResponse.refresh_token !== this.config.refreshToken) {
              ...
              try {
                ...
                this.lastPersistedSuffix = newTokenSuffix;
                this.lastPersistedAt = Date.now();
              } catch (persistError) {
  ...
  private async makeRequest<T>(method: string, path: string, data: unknown = null, retryCount = 0): Promise<T> {
    ...
    try {
      ...
      // If we get 401, try to refresh token and retry once. Not audit-logged:
      // a 401 here just triggers a normal refresh — if that refresh fails it is
      // logged as refresh_failure, which is the event that actually matters.
  ...
  async getAllDevices(): Promise<AirToAirUnit[]> {
    ...
    const collect = (buildings: Building[] | null): number => {
      let count = 0;
